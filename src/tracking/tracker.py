...
def fftd(img, backwards=False, byRow=False):
    # shape of img can be (m,n), (m,n,1) or (m,n,2) in my test, fft provided by numpy and scipy are slower than
    # cv2.dft return cv2.dft(np.float32(img), flags=((cv2.DFT_INVERSE | cv2.DFT_SCALE) if backwards else
    # cv2.DFT_COMPLEX_OUTPUT))  # 'flags =' is necessary! DFT_INVERSE: 用一维或二维逆变换取代默认的正向变换, DFT_SCALE:
    # 缩放比例标识符，根据数据元素个数平均求出其缩放结果，如有N个元素，则输出结果以1/N缩放输出，常与DFT_INVERSE搭配使用。 DFT_COMPLEX_OUTPUT:
    # 对一维或二维的实数数组进行正向变换，这样的结果虽然是复数阵列，但拥有复数的共轭对称性
...
def limit(rect, limits):
    ...
    if rect[0] < limits[0]:
        rect[2] -= (limits[0] - rect[0])
        rect[0] = limits[0]
    if rect[1] < limits[1]:
        rect[3] -= (limits[1] - rect[1])
        rect[1] = limits[1]
    if rect[2] < 0:
        rect[2] = 0
    if rect[3] < 0:
...
class KCFTracker:
    def __init__(self, hog=False, fixed_window=True, multi_scale=False):
        ...
        self._tmpl = None  # numpy.ndarray raw: (size_patch[0], size_patch[1]) hog: (size_patch[2], size_patch[
        # 0]*size_patch[1])
