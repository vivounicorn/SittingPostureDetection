...
def func1(dx, dy, boundary_x, boundary_y, height, width, num_channels):
    ...
    for j in xrange(1, height - 1):
        for i in xrange(1, width - 1):
            ...
            for ch in xrange(1, num_channels):
                tx = dx[j, i, ch]
                ty = dy[j, i, ch]
                ...
                for jj in xrange(k):
                    if ((i * k + ii > 0) and (i * k + ii < height - 1) and (j * k + jj > 0) and (
                        ...
                        mapp[i * stringSize + j * p + alfa[k * i + ii, j * k + jj, 0]] += r[k * i + ii, j * k + jj] * w[
                            ii, 0] * w[jj, 0]
                        mapp[i * stringSize + j * p + alfa[k * i + ii, j * k + jj, 1] + NUM_SECTOR] += r[k * i + ii, j * k + jj] * \
                                                                                                       w[ii, 0] * w[jj, 0]
                        if (i + nearest[ii] >= 0) and (i + nearest[ii] <= sizeY - 1):
                            mapp[(i + nearest[ii]) * stringSize + j * p + alfa[k * i + ii, j * k + jj, 0]] += r[k * i + ii, j * k + jj] * \
                                                                                                              w[ii, 1] * w[jj, 0]
                            mapp[(i + nearest[ii]) * stringSize + j * p + alfa[
                                k * i + ii, j * k + jj, 1] + NUM_SECTOR] += r[k * i + ii, j * k + jj] * w[ii, 1] * w[jj, 0]
                        if (j + nearest[jj] >= 0) and (j + nearest[jj] <= sizeX - 1):
                            mapp[i * stringSize + (j + nearest[jj]) * p + alfa[k * i + ii, j * k + jj, 0]] += r[k * i + ii, j * k + jj] * \
                                                                                                              w[ii, 0] * w[jj, 1]
...
def func3(part_of_norm, mappmap, sizeX, sizeY, p, xp, pp):
    ...
    for i in xrange(1, sizeY + 1):
        for j in xrange(1, sizeX + 1):
            ...
            val_of_norm = np.sqrt(part_of_norm[i * (sizeX + 2) + j] +
...
def getFeatureMaps(image, k, mapp):
    ...
    # 200x speedup
    r, alfa = func1(dx, dy, boundary_x, boundary_y, height, width, num_channels)  # with @jit
    # ~0.001s
    ...
    # 500x speedup
    mapp['map'] = func2(r, alfa, nearest, w, k, height, width, sizeX, sizeY, p,
                        stringSize)  # with @jit
    # ~0.001s
...
def normalizeAndTruncate(mapp, alfa):
    ...
    # 30x speedup
    new_data = func3(partOfNorm, mapp['map'], sizeX, sizeY, p, xp, pp)  # with @jit
